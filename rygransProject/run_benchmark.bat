...
:: יצירת שם ייחודי לתיקייה לפי תאריך ושעה (כדי לא לדרוס ריצות קודמות)
...
for %%f in ("%TEST_DIR%\*") do (
    ...
    :: שינוי: הקובץ הדחוס יישמר בתוך התיקייה החדשה שיצרנו
    ...
    if exist "!OUTPUT_FILE!" (
        ...
        echo Processed: !FILENAME_ONLY! (Saved in !OUTPUT_DIR!)
        
        :: הערה: ביטלנו את המחיקה כדי שהקבצים יישארו בתיקייה
        :: del "!OUTPUT_FILE!"
    )
)
...
echo.
echo Done. Created folder: !OUTPUT_DIR!
start "" %LOG_FILE%
endlocal